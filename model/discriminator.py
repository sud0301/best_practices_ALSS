...
class S4GAN_D(nn.Module):

    def __init__(self, num_classes, dataset, ndf = 64):
        super(S4GAN_D, self).__init__()
        ...
        if dataset == 'pascal_voc' or dataset == 'brain_slice':
            self.avgpool = nn.AvgPool2d((20,20))
        elif dataset == 'cityscapes' or dataset == 'a2d2':
...
def s4GAN_discriminator(num_classes, dataset, ndf=64):
    return S4GAN_D(num_classes, dataset, ndf)
