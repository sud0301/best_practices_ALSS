...
class VOCOracleGTDataSet(data.Dataset):
    def __init__(self, oracle, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
        self.root = root
        self.oracle = oracle
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, "JPEGImages/%s.jpg" % name)
            label_file = osp.join(self.oracle, "%s.png" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class VOCGTTDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
        self.root = root
        self.list_path = list_path
