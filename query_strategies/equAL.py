...
from data.dataloaders.pascal_voc import VOCDataSet
...
class EquAL(Strategy):
    def __init__(self, list_train_imgs, idxs_lb, idxs_unlb, net, args, writer):
        super(EquAL, self).__init__(list_train_imgs, idxs_lb, idxs_unlb, net, args, writer)
    ...
    def entropy_image_avg(self, pred):
        b = F.softmax(pred, dim=0)*F.log_softmax(pred, dim=0)
        b = -1.0 * b.sum(dim=0)
        return torch.sum(b).cpu().numpy()/(b.size(0)*b.size(1))

    def compute_incons(self, diff):
    ...
    def sort_images_by_entropy(self):
        ...
        with torch.no_grad(): 
            for ind, batch in enumerate(trainloader):
                ...
                output_flip  = self.net(image_flip.cuda())
                output_flip = interp(output_flip).squeeze(0)
                output_flip = output_flip[:,:size[0],:size[1]]

                ent = self.entropy_image_avg(output)
                ent_flip  = self.entropy_image_avg(output_flip)
                
                ent_sum = ent + ent_flip

                if ind%500==0:
                    print (ind)
            
                list_index_ent.append((index, ent_sum))
